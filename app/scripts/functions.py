...
def new_object_over_type(type_object: str, input_object: Message, standart: dict) -> Union[bool, str]:                    
...
def validate_input_kick_user_index(kick_user_id: str, max_users_id: int) -> Tuple[bool, str]:
    if not kick_user_id.isdecimal():
        return (False, 'no_number')
    if kick_user_id == '0':
        return (False, 'kick_admin')
    kick_user_id = int(kick_user_id)
    if kick_user_id > max_users_id:
